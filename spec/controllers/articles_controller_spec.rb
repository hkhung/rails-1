...
describe ArticlesController, type: :controller do
	...
	describe '#like' do
		...
		it 'votes like to the article' do
			expect{ do_request }.to change{ Vote.count }.from(0).to(1) 
			expect(response).to redirect_to articles_path
		end
	end
	...
	describe '#dislike' do
		...
		it 'votes dislike to the article' do
			expect{ do_request }.to change{ Vote.count }.from(0).to(1)
			expect(response).to redirect_to articles_path
		end
	end
	...
	describe '#unvote' do
		...
		it 'votes dislike to the article' do
			vote
			expect{ do_request }.to change{ Vote.count }.from(1).to(0)
